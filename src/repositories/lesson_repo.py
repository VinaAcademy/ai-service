...
class LessonRepository(BaseRepository[Lesson]):
    ...
    async def get_lesson_with_course_context(
            self, lesson_id: UUID, include_deleted: bool = False
    ) -> Optional[dict]:
        """
        Get a specific lesson by ID with full course context.

        This method returns a single lesson with course and section information:
        - course name, description, language, level
        - section title
        - lesson type, description, title
        ...
        Returns:
            Dictionary with course, section, and lesson info or None

        SQL equivalent:
            SELECT c.name, c.description, c.language, c.level,
                   s.title, l.lesson_type, l.description, l.title
            FROM lessons l
